...
export default class CanvasSelect extends EventBus {
    ...
    isStraigeLineX = false

    isStraigeLineY = false
    /**
            ...
            if (this.ctrlIndex > -1) { // 点击到控制点
                ...
                if (this.activeShape.creating) {// 创建中
                    if ([2, 4].includes(this.activeShape.type)) {
                        if(this.activeShape.type === 2 && this.isPolygonStartPoint ){
                            this.activeShape.coor.push(this.activeShape.coor[0]);
                            this.emit('add', this.activeShape);
                            this.activeShape.creating = false
                            this.isPolygonStartPoint = false
                        }else {
                            const [x, y] = this.activeShape.coor[this.activeShape.coor.length - 1];
                            if (x !== offsetX && y !== offsetY) {
                                let nx = Math.round(offsetX - this.originX / this.scale)
                                let ny = Math.round(offsetY - this.originY / this.scale)
                                if(this.isStraigeLineY){
                                    nx = this.activeShape.coor[this.activeShape.coor.length - 1][0]
                                }else if(this.isStraigeLineX){
                                    ny = this.activeShape.coor[this.activeShape.coor.length - 1][1]
                                }
                    ...
                    console.warn("hitShapeIndex",hitShapeIndex)
                    console.warn("hitShape",hitShape)
    ...
    handelMouseMove(e: MouseEvent | TouchEvent) {
        ...
        if (this.lock) return;
        const { mouseX, mouseY, mouseCX, mouseCY } = this.mergeEvent(e)
        console.log("0mouseX："+mouseX);
        console.log("0mouseY："+mouseY);
        console.log("0mouseCX："+mouseCX);
        console.log("0mouseCY："+mouseCY);
        const offsetX = Math.round(mouseX / this.scale);
        const offsetY = Math.round(mouseY / this.scale);
        console.log("0offsetX："+offsetX);
        console.log("0offsetY："+offsetY);
        console.log("0originX："+this.originX);
        console.log("0originY："+this.originY);
        this.mouse = this.isMobile && (e as TouchEvent).touches.length === 2 ? [mouseCX, mouseCY] : [mouseX, mouseY];
        if (((!this.isMobile && (e as MouseEvent).buttons === 1) || (this.isMobile && (e as TouchEvent).touches.length === 1)) && this.activeShape.type) {
            ...
            //
            //
            // if(e.ctrlKey && (Math.abs(this.activeShape.coor[0][0] - nx) - Math.abs(this.activeShape.coor[0][1] - ny) >0 ){
            //     this.
            // }
            this.isStraigeLineY = false
            this.isStraigeLineX = false
            const [x, y] = this.activeShape.coor[this.activeShape.coor.length - 1];
            if (x !== offsetX && y !== offsetY) {
                const nx = Math.round(offsetX - this.originX / this.scale)
                const ny = Math.round(offsetY - this.originY / this.scale)
            if(e.ctrlKey){
                if( Math.abs(x - nx) - Math.abs(y - ny) > 0){
                    this.isStraigeLineX = true
                    this.isStraigeLineY = false
                } else {
                    this.isStraigeLineX = false
                    this.isStraigeLineY = true
                }
            }
            ...
            if(this.activeShape.coor.length > 2){
                ...
                //     const ny = Math.round(offsetY - this.originY / this.scale)
                    ...
                    //     console.log("mouseCX："+mouseCX);
                    //     console.log("mouseY："+mouseY);
                    //     console.log("mouseCY："+mouseCY);
                    //     // const offsetX = Math.round(mouseX / this.scale);
                    //     // const offsetY = Math.round(mouseY / this.scale);
                    ...
                    //
                    //     // console.log("后offsetX："+offsetX);
                    //     // console.log("后offsetY："+offsetY);
                    //     // console.log("后originX："+this.originX);
                    //     // console.log("后originY："+this.originY);
                    //     // const endX = mouseCX- this.canvas.getBoundingClientRect().left;
                    //     // const endY = mouseCY -this.canvas.getBoundingClientRect().top;
                    //     const deltaX = Math.abs(endX - this.originX);
                    //     const deltaY = Math.abs(endY - this.originY);
                    //     if (deltaX > deltaY) {
                    //         // Draw a horizontal line (difference in X greater than Y)
                    //         nx =  ;
                    //         console.log("nx:"+nx)
                    //         // Draw the horizontal line
                    //         //this.drawLine(startX, startY, endX, startY);
                    //     } else {
                    //         // Draw a vertical line (difference in Y greater than X)
                    //         ny = deltaY;
                    //         console.log("ny:"+ny)
                    ...
                    if(Math.abs(this.activeShape.coor[0][0] - nx) < 6  && Math.abs(this.activeShape.coor[0][1] - ny) < 6){
                        this.isPolygonStartPoint = true
                    }else {
                        this.isPolygonStartPoint = false
                    }
                }
    ...
    handelRightClick(e: MouseEvent | TouchEvent){
        console.warn('')
    }
    ...
    //             this.transformShape('copy')
    //         }else if(e.code === 'KeyX' && e.ctrlKey){
    //             this.transformShape('upsideDown')
    //         }else if(e.code === 'KeyB' && e.ctrlKey){
    //             this.transformShape('leftAndRight')
    //         }
    //     }
    // }
     ...
     */
    transformShape(transformationType:string){
        let newShape;
        let coor: Point[] = [];
        if(transformationType == 'copy') {
            if ([3, 5].includes(this.activeShape.type)) {
                const [x, y] = this.activeShape.coor;
                coor = [[x + 10 ,  y + 10]];
            } else {
                this.activeShape.coor.forEach((pt: any) => {
                    coor.push([pt[0] - 10, pt[1] -10]);
                });
            }
        }else if(transformationType == 'upsideDown'){
            if ([3, 5].includes(this.activeShape.type)) {
                const [x, y] = this.activeShape.coor;
                coor = [[x + 10,  y + 10]];
            } else if(this.activeShape.type ==1 ){
                this.activeShape.coor.forEach((pt: any) => {
                    coor.push([pt[0] - 10, pt[1] -10]);
                });
            }else {
                ...
                const h = minY + maxY;
                this.activeShape.coor.forEach((pt: any) => {
                    coor.push([pt[0], h - pt[1]]);
                });
            }
        }else if(transformationType == 'leftAndRight'){
            if ([3, 5].includes(this.activeShape.type)) {
                const [x, y] = this.activeShape.coor;
                coor = [[x + 10, y + 10]];
            }else if(this.activeShape.type ==1 ){
        ...
        switch (this.activeShape.type) {
            case 1:
                newShape = new Rect({ coor: [coor[0],coor[1]]}, this.dataset.length);
                newShape.label = this.activeShape.label;
                newShape.creating = false;
                break;
            case 2:
                newShape = new Polygon({ coor: coor }, this.dataset.length);
                newShape.label = this.activeShape.label;
                newShape.creating = false;
                break;
            case 3:
                newShape = new Dot({ coor: coor[0] }, this.dataset.length);
                newShape.label = this.activeShape.label;
                this.emit('add', newShape)
                break;
            case 4:
                newShape = new Line({ coor: coor }, this.dataset.length);
                newShape.label = this.activeShape.label;
                newShape.creating = false;
                break;
            case 5:
                newShape = new Circle({ coor: coor[0] }, this.dataset.length);
                newShape.label = this.activeShape.label;
                newShape.radius = this.activeShape.radius
                newShape.creating = false;
                break;
            default:
                break;
...
/**
    ...
    isMouseInshape(e: MouseEvent | TouchEvent) {
        e.stopPropagation();
        this.evt = e
        if (this.lock) return;
        const { mouseX, mouseY, mouseCX, mouseCY } = this.mergeEvent(e)
    ...
    drawPolygon(shape: Polygon) {
        ...
        if (creating) {
            ...
            if(this.isStraigeLineX) {
                    this.ctx.lineTo(x - this.originX, lastY)
            } else if(this.isStraigeLineY) {
                this.ctx.lineTo(lastX, y - this.originY)
            } else {
     ...
     * @param point 坐标
     */
    drawPolygonStartPoint(point: Point) {
        const [x, y] = point.map((a) => a * this.scale);
        this.ctx.save();
        this.ctx.beginPath();
        this.ctx.fillStyle = this.ctrlFillStyle;
        this.ctx.strokeStyle = this.ctrlStrokeStyle;
        this.ctx.arc(x, y, 8, 0, 2 * Math.PI, true);
        this.ctx.fill();
        this.ctx.arc(x, y, 8, 0, 2 * Math.PI, true);
        this.ctx.stroke();
        this.ctx.restore();
    ...
    drawCtrlList(shape: Rect | Polygon | Line) {
        shape.ctrlsData.forEach((point, i) => {
            if (shape.type === 5) {
                ...
                if(shape.type === 2 && this.isPolygonStartPoint && i ===0 ){
                    this.drawPolygonStartPoint(point)
                }else {
                    this.drawCtrl(point);
                }
            }
