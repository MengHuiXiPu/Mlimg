<template>
  ...
  >
    ...
    <a-form-model
      ...
      layout="horizontal"
    >
      <a-form-model-item label="任务名称" prop="createTaskName">
        <a-input v-model="form.createTaskName" />
      </a-form-model-item>
      <a-form-model-item label="任务描述" prop="createTaskDesc">
        <a-input v-model="form.createTaskDesc" type="textarea" />
      </a-form-model-item>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        createTaskName: [{ required: true, message: '请输入任务名称', trigger: 'blur' }],
        createTaskDesc: [{ required: false, message: '请输入任务描述', trigger: 'blur' }],
    ...
    handleOk () {
      this.$refs.ruleForm.validate(async (valid) => {
        if (!valid) {
          ...
          const res = await addTaskFromExistConfig(params)
          this.loading = false
