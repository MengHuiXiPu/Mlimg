  ...
  <div class="app-container">
    ...
    <div class="toolbar space-between">
      ...
      <div style="margin-bottom: 18px;">
        <el-button type="primary" @click="crud.toAdd()" icon="el-icon-plus">新建菜单</el-button>
    ...
    >
      ...
      <el-table-column align="" prop="icon" label="菜单名称" min-width="120">
        <template slot-scope="scope">
          <IconFont v-if="scope.row.icon" :type="scope.row.icon" />
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column align="" show-overflow-tooltip prop="path" label="路由地址" />
      <el-table-column align="" show-overflow-tooltip prop="componentName" label="路由名称" />
      <el-table-column align="" show-overflow-tooltip prop="component" label="组件路径" />
      <el-table-column align="" show-overflow-tooltip prop="layout" label="页面布局">
        <template slot-scope="scope">
          {{ dict.label.Layout[scope.row.layout] }}
        </template>
      </el-table-column>

      <el-table-column align="" show-overflow-tooltip prop="permission" label="权限标识" />
      <el-table-column align="" prop="hidden" label="隐藏" width="60">
        <template slot-scope="scope">
          {{ scope.row.type === 1 ? (scope.row.hidden ? '是' : '否') : '--' }}
        </template>
      </el-table-column>
      <el-table-column prop="sort" align="right" label="排序" width="60">
        <template slot-scope="scope">
          {{ scope.row.sort }}
        </template>
      </el-table-column>
      <el-table-column align="" prop="createTime" label="创建时间" width="160">
        <template slot-scope="scope">
...
import { validateName, validateString, validateJSON } from '@/utils';
