      ...
      draw(cfg, group) {
        ...
        if (end.x > start.x) {
          path = [
            ...
            [
              'C',
              start.x + (end.y > start.y? 0 : (cfg.start.x < 0 ? -40: 40)),
              start.y + hgap / (hgap / 50),
              end.x + (end.y > start.y? 0 : (cfg.end.x < 0 ? -40: 40)),
              end.y - hgap / (hgap / 50),
              end.x + (end.y > start.y? 0 : (cfg.end.x < 0 ? -40: 40)),
              end.y - 4
            ],
        ...
        const keyShape = group.addShape('path', {
          attrs: {
            id: 'edge' + uniqueId(),
            path: path,
            stroke: '#888888',  //连线颜色  b8c3ce
            lineAppendWidth: 10,
            lineWidth: 1.5,   //修改连线宽度
            endArrow: {
              path: endArrowPath,
            }
          }
