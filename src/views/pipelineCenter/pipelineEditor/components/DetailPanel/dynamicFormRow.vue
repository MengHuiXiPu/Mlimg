      ...
      <el-col :span="18">
        <template v-if="item.controlType === 'textInput'">
          <div class="vertical-center"  v-for="(inheritVariable, idx) in getInitInheritVariables(item.inheritVariables)" :key="idx">
            ...
            />
            <span
              v-for="(variable, index) in inheritVariable"
              :key="index"
            >
            ...
            <el-button
              v-if="inheritVariable && inheritVariable.length > 0"
              @click="clearInherit(item, getInheritNodeIdFromTemp(item.inheritNodeIds , idx))"
              icon="el-icon-delete"
              type="text"
            ></el-button>
            <el-button
        ...
        <template v-else-if="item.controlType === 'slider'">
          <div class="vertical-center" v-for="(inheritVariable, idx) in getInitInheritVariables(item.inheritVariables)" :key="idx">
            <el-input-number
              ...
              :max="item.maxValue"
              :precision="
                getPrecision(item.step || values[prefixName(item.varName)])
              "
              @change="handleSelectChange(prefixName(item.varName), hasInherit)"
        ...
        <template v-else-if="item.controlType === 'fileUpload'">
          ...
          <div v-if="item.fileOption.filterName ==='onnx'" class="select-model-wrap">
            ...
            <el-popover 
              ...
              :content="item.value&&item.value.endsWith('.onnx') ? item.value: ''"
            >
...
import { arraysEqual } from "@/views/pipelineCenter/pipelineEditor/utils";
...
export default {
  ...
  mounted() {
    this.uploadId = new Blob([this.G6Instance.pipelineId], {
  ...
  methods: {
    ...
    openDialog(varName, nodeId, variable) {
      ...
      this.inheritEdge.forEach((edge) => {
        ...
        if (
          obj.sourceId == ele.inheritNodeId &&
          obj.targetId == this.node.id &&
          this.inheritanceForm.inheritVariable.includes(obj.startVar) &&
          obj.endVar == varName
        ) {
      ...
      const traverse = (node, parentPath = "") => {
        if (node.varName) {
          const fullPath = parentPath
            ...
            : node.varName;
          if (
            (arraysEqual(node.struct, this.inheritanceForm.struct) &&
              this.inheritanceForm.variableType == node.varType) ||
    ...
    async callOnnxModelSelector(item) {
      ...
      try {
        ...
        const bindResult = await pipelineApi.bindOperatorModelPipeline(this.pipelineId,{
