<template>
  <div class="toolbar">
    ...
    <i
      ...
      title="撤销"
      :class="!readOnly && undoList.length > 0 ? '' : 'disable'"
      @click="handleUndo"
    ></i>
    <i
      class="command iconfont icon-redo"
      title="重做"
      :class="!readOnly && redoList.length > 0 ? '' : 'disable'"
      @click="handleRedo"
    ></i>
    ...
    <i
      ...
      title="删除"
      :class="!readOnly && selectedItem ? '' : 'disable'"
      @click="handleDelete"
    ></i>
    <span class="separator"></span>
    <i
    ...
    <i
      ...
      title="成组"
      :class="!readOnly && addGroup ? '' : 'disable'"
    ...
    <i
      ...
      title="解组"
    ></i>
    <el-button @click="saveData" :loading="saveLoading" type="primary" :disabled="readOnly" style="margin-left: 15px" size="mini"
      >保存</el-button
    >
    <el-button @click="startDebugging" :disabled="readOnly || !filePath" :loading="debuggingLoading" type="primary" size="mini">调试</el-button>
...
import {uniqueId, getBox, toLabelType} from "../../utils";
...
export default {
  ...
  methods: {
    ...
    bindEvent() {
      let self = this;
      eventBus.$on("afterAddPageGroup", (page) => {
        self.page = page;
    ...
    saveData() {
      ...
      const id = this.$parent.operatorGroupId;
      ...
      // 保存算子组
      return Operator.updateOperatorGroupTemplateById(id, data)
        .then((res) => {
    ...
    async startDebugging() {
      if (!this.filePath || this.readOnly) {
        return;
      }
      await this.saveData()
      // 调试算子组
      Operator.operatorGroupDebug(this.operatorGroupId, {
  ...
  beforeDestroy() {
    // 务必移除事件监听
    eventBus.$off('afterAddPageGroup')
    eventBus.$off('add')
    eventBus.$off('update')
    eventBus.$off('delete')
