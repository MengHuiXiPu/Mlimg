  ...
  methods: {
    bindEvent() {
      ...
      eventBus.$on("nodeselectchange", (item) => {
        if (item.select === true && item.target.getType() === "node") {
          ...
          self.$set(self, "node", item.target.getModel());
          // this.$forceUpdate();
        }else {
          //没有选中节点，这里应该清空node
          this.node = null
        ...
        let value = null;
        // if (!self.node.result && ele != null && ele.value) {  //fileUpload的算子输出，加载value对应的图片并展示
        if (!self.node.result && outputEle != null && outputEle.value) {  //fileUpload的算子输出，加载value对应的图片并展示
          this.fileUpload = true;
          value = outputEle.value;
          if(typeof(value) ==='string' && value.endsWith('.npy') ) return
          // 算子组获取图片
          this.imageLoader.loadImage(Operator.getGroupImage, {
    ...
    setFieldValue(output, structFieldName, structFieldValue, modelName) {
      ...
      this.imageLoader.loadImage(Operator.getGroupImage, {
        ...
        // this.$refs.outputsItemComp.$forceUpdate()
        // this.$set(output, 'value', imgUrl)
        // this.$forceUpdate();
      })
    },
  },
