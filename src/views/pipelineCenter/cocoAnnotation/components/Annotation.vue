    ...
    >
      ...
      <!-- <el-tooltip effect="dark" :content="getValue" placement="top">
        ...
        <button
          ...
          @click.stop="onAnnotationClick($event, !showKeypoints)"
        >
          {{ annotation.name }}
        </button>
        <!-- 锁定算子 -->
        <!-- <i :class="annotation.lockValue ? 'el-icon-lock': 'el-icon-unlock'" @click="toggleLockStatus(annotation)"></i> -->
        <a-switch v-model="annotation.lockValue" checked-children="lock" size="small"></a-switch>
    </li>
    <div
      class="modal fade"
      tabindex="-1"
...
export default {
  ...
  methods: {
    ...
    onAnnotationClick($event={}, showKeypoints) {
      let { imageId,  segmentation = []} = this.annotation
      //  && this.annotation.labelingType !== 'point'
      if (this.annotation.labelingType !== 'roi') {
        return
      }
      ...
      if (imageId && $event.isTrusted) { //关联了图片 且用户点击行为
        this.$emit("click-annotation", this.index);
      }else { //没关联图片时，也就不需要联动图片聚焦了，此时仍然保留之前代码的click事件
        this.$emit('click', this.index)
      }
    },
    ...
    addKeypoint(point, visibility, label) {
      ...
      let keypoint = new Keypoint(point.x, point.y, {
        ...
        onClick: (event) => {
          ...
          this.onAnnotationClick({isTrusted: false}, true);
  ...
  computed: {
    // getValue() {
    //   return this.argValue || this.annotation.defaultValue;
    // },
    categoryIndex() {
  ...
  mounted() {
    this.initAnnotation();
    // switch (this.annotation.labelingType) {
    //   case "roi":
    //     eventBus.$on("on-roi", (arg) => {
    //       this.argValue = arg;
    //     });
    //     break;
    //   case "point":
    //     eventBus.$on("on-point", (arg) => {
    //       this.argValue = arg;
    //     });
    //     break;
    //   case "template":
    //     eventBus.$on("on-template", (arg) => {
    //       this.argValue = arg;
    //     });
    //     break;
    // }
