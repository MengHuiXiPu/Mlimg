  ...
  <div class="sample-detail">
    <a-spin :spinning="exportLoading" tip="正在准备文件中，请稍等...">
      ...
      <div class="sample-detail-image">
        <div class="sample-detail-left scrollbar">
          <div class="sample-detail-title">图片标签列表</div>
          <div class="search-sit">
            <a-input-search placeholder="请输入类别名称" @search="onSearch" @change="onSearchChange" enter-button="查询" size="default"/>
          </div>
          <ul class="left-list scrollbar">
            <div>
              <a-checkbox v-if="exportType" :indeterminate="indeterminate" :checked="checkAll" @change="changeSelectAll">全选</a-checkbox>
            </div>
        ...
        <div class="sample-detail-right scrollbar">
          <div class="sample-detail-header">
            <div class="sample-detail-group" v-show="editType">
              <a-button-group size="small">
                <a-upload :accept="accept.toString()" :showUploadList="false" :before-upload="uploadImage">
                  <a-button type="default" size="small">上传<a-icon type="upload" style="vertical-align: unset"></a-icon></a-button>
                </a-upload>
                ...
                <a-button type="primary" @click="handleMoveImage" :disabled="selectImageList.length === 0 && selectCodeList.length === 0">
                  移动图片到<a-icon type="arrow-right" style="vertical-align: unset"/>
                </a-button>
              </a-button-group>
            </div>
            <div class="sample-detail-group" v-show="exportType">
              <a-button-group>
                <a-tooltip>
                  <template slot="title" v-if="selectCodeList.length === 0">请先选择类别</template>
                  <a-button type="default" @click="exportDataSet" :disabled="selectCodeList.length === 0">导出</a-button>
                </a-tooltip>
              </a-button-group>
            </div>
            <div class="sample-detail-check">
              <a-checkbox :checked="exportType" @change="changeExportType" v-if="!readOnly">导出模式</a-checkbox>
          ...
          <div class="sample-detail-right-content">
            ...
            <a-spin :spinning="imgListLoading">
              <div class="img-item" v-for="pictureItem in pictureList" :key="pictureItem.id">
                <img :id="'img_' + pictureItem.id" :src="(labelType ? picIdUrlBboxMap[pictureItem.id] : picIdUrlMap[pictureItem.id]) || defaultPhone" style="border-radius: 4px;"
                  @click="handleShowLargePicture(pictureItem)" />
                <!-- 显示空白复选框 ———— 编辑模式开启 || 没有选中任何类别 -->
                <div class="selectBtn" v-if="editType || selectCodeList.length !== 0">
                  <a-checkbox :checked="pictureItem.selected" @change="selectImage($event, pictureItem)" :disabled="pictureItem.disabledCheck"></a-checkbox>
                </div>
                <!-- <a-icon v-if="!picIdUrlMap[pictureItem.id]" type="loading" /> -->
                <a-icon v-if="labelType ? !picIdUrlBboxMap[pictureItem.id] : !picIdUrlMap[pictureItem.id]" type="loading" />
                <!-- <svg :xmlns="'http://www.w3.org/2000/svg'" v-show="labelType" version="1.1" :id="'svg' + pictureItem.id"
                  @click="handleShowLargePicture(pictureItem.id)"></svg> -->
              </div>
              ...
              <!-- <a-empty v-if="!pictureList.length" /> -->
            </a-spin>
          </div>
          <a-empty v-if="!imgListLoading && !pictureList.length" />
          <div class="pagation-image" v-if="pictureList.length > 0" >
    ...
    <AnnotatorModal  
      ...
      @on-fresh="reloadPage" />
    <!-- 图片移动 -->
    <move-image-modal :visible.sync="showMoveModal" v-if="showMoveModal" :datasetId="currentID" :markRange="markRange" @success="afterMoved" ref="refMoveImageModal"></move-image-modal>
  </div>
</template>
...
import { getFileCategoryAndCount, getSearchCategory, getPicture, downloadFileCommon } from "@/api/modelManage"
export default {
  name: 'sampleDetail',
  components: {
    ImagePreview,
  ...
  data() {
    return {
      ...
      showMoveModal: false, // 是否显示移动图片弹窗
    }
  },
  computed: {
    // 获取已勾选图片列表id
  ...
  methods: {
    ...
    handleMoveImage() {
      ...
      const selectedMenus = this.menuData.filter(item => item.selected === true)
      // 理论上只能选择一个目录，但鬼知道这代码靠不靠谱
      if (selectedMenus.length > 1) return this.$message.warning('仅支持移动单一目录')
      const params = {
        sourceLabelName: "",
        disabledLabelName: "",
        picIds: [], //
        dlTagType: this.detailData.dlTagType
      }
      if (selectedMenus.length === 1) { //移动整个label下的所有图片
        params.sourceLabelName = selectedMenus[0].name
        params.disabledLabelName = selectedMenus[0].name
      } else { //获取具体要移动的图片id
        params.picIds = this.selectImageList
        // 当前图片所在的label要禁止在移动时选择
        params.disabledLabelName = this.selectMenu
      }
      this.$nextTick(() => {
        this.$refs.refMoveImageModal.initState(params)
      })
    },
    /**
     * @description: 图片移动完成后执行的刷新操作（摘自原有sanpleDetail.vue的moveTo方法的原有逻辑）
     */
    afterMoved() {
    ...
    */
    changeSelectCode(e, name) {
      ...
      this.pictureList.forEach((item, index) => {
        if (item.categoryName === name) {
          item.selected = selected;
          // 选中类别后，在编辑模式下不再允许单独取消勾选某张图片
          if (this.editType) {
            this.$set(item, 'disabledCheck', selected);
          }
        }
      });
    },
    ...
    selectAllImg() {
      ...
      this.menuData.forEach(item => {
        if (item.name === categoryName) {
          ...
          item.disabled = this.selectAllImgType
        }
      })
      this.pictureList.forEach((item, index) => {
        // fix: 点击这个全选按钮，会切换对应menu的勾选状态，但不会触发change事件，不会执行changeSelectCode， 也即不会解锁图片的disableCheck，导致不能选择图片
     ...
     * 点击某张图片的事件handler
     */
    handleShowLargePicture(pictureItem) {
      if (this.readOnly) return
      const imageID = pictureItem.id
      if (this.editType) {
        this.pictureList.forEach((item, index) => {
          if (item.id === imageID) {
            if (!item.disabledCheck) {  //非禁用选中状态下才可以操作
              item.selected = !item.selected
              this.$set(this.pictureList, index, item)
            } else {
              this.$message.warning('编辑模式下，勾选标签后不允许单独选中图片，如需操作单张图片请取消勾选整个标签！')
            }
          }
        })
        // // 获取已选中的图片数，如果等于pictureList.length,说明是全选，需要选中
        // if()
      } else {
