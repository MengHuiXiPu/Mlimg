<template>
  ...
  >
    <a-form-model
      ...
      <a-form-model-item label="任务描述" prop ="algorithmDescribe" v-if="!this.formData.id">
...
import { saveTask } from "@/api/atsMap.js";
export default {
  data() {
    return {
  ...
  computed: {
    modalTitle() {
      return this.formData.id ? '更新任务' : '新建任务'
    }
  },
  props: {
    visible: {
      ...
      this.$refs.refConfigForm.validate(valid => {
        ...
        saveTask(params).then(res => {
          if (res?.code === 0) {
            ...
            this.handleCancel()
            const targetId = this.formData.id || res.data?.id
            this.$emit("success", targetId)
          } else {
