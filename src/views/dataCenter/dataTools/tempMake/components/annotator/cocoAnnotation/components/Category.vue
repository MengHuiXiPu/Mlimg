    ...
    <div v-show="mode == 'label'">
      <div class="label-list">
        <CLabel
          ...
          :category="category"
          :hasSelected="hasSelected.flag"
          :annotationId="hasSelected.id"
          :search="categorysearch"
          @on-selected="onUpdate"
        />
      </div>
      <label-count
        v-for="item in LabelCountStat"
        :key="item.id"
        :count="item.count"
        :labelName="item.labelName"
        :maxValue="maxLabelCount"
      />
    </div>
    <!-- 标签管理编辑标签弹窗 -->
    <div
...
import labelCount from "./tools/labelCount.vue";
...
export default {
  ...
  methods: {
    openCaModal() {
      ...
      // 此if为兼容数据管理中的标注功能
      if(this.$parent.noDatasetMode) { //将label数据特殊保存处理
        return
      }
    ...
    getLabelAnnotationCount() {
      axios.get(`/api/v1/datacenter/dataListAnnotation/getAllLabelCountStatsByDatasetId/${this.$parent.dataset.id}`).then(res => {
    ...
    getLabelCount() {
      axios
        .get(
          `/api/v1/datacenter/dataListAnnotation/getLabelCountStatsByDatasetId/${this.$parent.dataset.id}`
        )
        .then((res) => {
          this.LabelCountStat = res.data.filter((item) => item.count);
          const values = this.LabelCountStat.map((item) => item.count);
          this.maxLabelCount = Math.max(...values);
        });
    },
    ...
    deleteItem(row) {
      ...
      // 以下为数据集标注原本逻辑
      const targetLabelDetail = this.allLabelAnnotationCount.find(lab => lab.id === row.id) || {}
      if(targetLabelDetail.count && targetLabelDetail.count > 0) {  //说明该标签下有标注，不能删除
    ...
    createAnnotation(callback, labelId, shape) {
      ...
      if(parent.noDatasetMode) {
        ...
        // setTimeout(() => {
          const annotaitonId =parseInt(uniqueId() + (Date.now().toString()).substr(-7))
          ...
          this.selectedAnnotation = this.getIndex(annotaitonId);
          this.$nextTick(() => {
      ...
      // 以下为数据集标注(原始功能逻辑)
      const loading = this.$GLoading('正在创建标注...')
      return axios
        ...
        .then((response) => {
          loading.close()
          const category = this.allCategories.find((item) => item.id === id);
          response.data.color = category.color || "#FF0000"  //不同的category下的 annotation 使用不同的颜色
          category.annotations.push(response.data);
          // let annotationId = this.category.annotations.length;
          const targetId = response.data.id;
          this.selectedAnnotation = this.getIndex(targetId);
          this.$nextTick(() => {
            this.$parent.selectLastEditorTool();
            this.$emit("click", {
              annotation: this.selectedAnnotation,
          ...
          // this.isVisible = true;
          this.showAnnotations = true;
          // this.$refs.annotation.eventTask()
