<template>
  <div class="mb-8">
    <!-- <i
      ...
      @click="click"
    /> -->
    <hr v-if="$attrs['show-prompt-tool']" />
    <!-- 使用提示标注的nacos控制字段 -->
    <a-button type="ghost" class="lineBtn" size="small" @click="btnTrigger" v-if="$attrs['show-prompt-tool']" :disabled="$parent.mode !='segment'">
      <!-- <span>提示</span><br />
      <span>标注</span> -->
      <span>智能</span><br />
      <span>标注</span>
...
export default {
  ...
  methods: {
    ...
    btnTrigger() {
      ...
      // this.promptAnnotating();
      // const defaultCategory = this.$parent.defaultCategory;
      // if (defaultCategory) {
      //   this.promptAnnotating(defaultCategory);
      // } else {
