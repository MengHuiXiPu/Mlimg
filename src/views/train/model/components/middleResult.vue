    ...
    <a-spin :spinning="loading">
      ...
      <div class="middleResult-content">
        ...
        <div class="middleResult-content-left">
          ...
          <div class="middleResult-content-left-content scrollbar" ref="list">
            <ul>
              <el-tooltip :disabled="item.file_type != 3" effect="light" v-for="(item, index) in fileList" :key="index" content="暂不支持预览" placement="right">
                <li
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    ...
    next(vm => {
      ...
      vm.fileList = []
      vm.currentImage = {}
      vm.getModelSrc(id, type === 'model' ? 1 : 2)
    })
  },
  data () {
    return {
  ...
  methods: {
    selectedImage (record) {
      ...
      if (record.file_type === 1) {
        ...
        middleResult.getImage(params).then(res => {
...
.middleResult{
  ...
  .middleResult-content-left {
    background: #fff;
    border-radius: 16px;
    padding-top: 12px;
  }
  .middleResult-content-right {
    width: calc(80% - 16px);
    margin-left: 16px;
    background: #fff;
    border-radius: 16px;
