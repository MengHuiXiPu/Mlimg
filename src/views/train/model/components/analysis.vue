...
import { analysis } from '@/api/offlineForecast'
...
export default {
  ...
  methods: {
    async setting () {
      ...
      const res = await analysis.setCodeQuotaAsBest(params)
      if (res.code !== 0) return false
      this.$message.success('设置成功')
    },
    async getTableData (type) {
      this.loading = true
      const res = await analysis.getData({ forecastId: this.id })
      this.loading = false
      if (res.code !== 0) return false
    ...
    changeValue: debounce(async function (record) {
      ...
      this.loading = true
      const res = await analysis.updateSingleData(params)
      this.loading = false
      if (res.code !== 0) return false
    ...
    changeMeasureAccuracyValue: debounce(async function (record) {
      ...
      this.loading = true
      const res = await analysis.setCodeAccuracyValue(params)
      this.loading = false
      if (res.code !== 0) return false
      // this.$message.success('修改成功')
      this.settingAllValue(res.data)
    ...
    changeGlobal: debounce(async function () {
      ...
      this.loading = true
      const res = await analysis.updateGlobalData(params)
      this.loading = false
      if (res.code !== 0) return false
      // this.$message.success('修改成功')
      this.settingAllValue(res.data)
    ...
    exportData () {
      this.loading = true
      downloadFile(`/api/v1/traincenter/offlineForecastResultAnalyse/exporBestThresholdByForecastId?forecastId=${this.id}`)
      setTimeout(() => {
        this.loading = false
      }, 1000)
    }
