    ...
    <a-spin :spinning="loading">
      <a-form-model
        ...
        <a-form-model-item label="算法名称" prop="imageName">
          <a-input :maxLength="50" v-model="rowData.imageName" :disabled="this.parentData.isVersion || (Boolean(rowData.id) && isVersion)" />
        </a-form-model-item>
        ...
        <a-form-model-item label="算法url" v-else-if="rowData.algorithmFetchType === 2" prop="algorithmName">
          <a-input v-model="rowData.algorithmName" :disabled="true" class="algorithmName" style="margin-right: 16px; width: 60%"/>
          <a-upload :showUploadList="false" :before-upload="algorithmUpload" :disabled="Boolean(rowData.id)" style="20%">
            <a-button type="default" :disabled="Boolean(rowData.id)"><a-icon type="upload"></a-icon>上传文件</a-button>
          </a-upload>
        </a-form-model-item>
        <a-form-model-item label="请选择算法" prop="fromImageId" v-else>
          <a-select
            placeholder="请选择算法"
        ...
        <a-form-model-item label="请选择模型" prop="algorithmStoredDir" v-if="rowData.algorithmFetchType === 3">
          <a-select
            placeholder="请选择模型"
            v-model="rowData.algorithmStoredDir"
            :disabled="Boolean(rowData.id) || !rowData.fromImageId"
            style="width: 46%">
            <a-select-option v-for="item in modelInfo" :key="item.id" :value="item.id" :disabled="!item.atLeastOneAvl">
              {{ item.modelName }}
            </a-select-option>
          </a-select>
          <a-select
            placeholder="请选择版本"
            v-model="rowData.modelId"
            :disabled="Boolean(rowData.id)"
            style="width: 46%; margin-left:16px">
            <a-select-option v-for="item in mirrorChildrenList" :key="item.id" :value="item.id" :disabled="item.imageStatus !== 1">
              {{ item.versionLabel }}
        ...
        <a-form-model-item label="模型说明" prop="imagesDescribe">
          <a-input v-model="rowData.imagesDescribe" type="textarea" />
        </a-form-model-item>

        <a-form-model-item label="模型类型" prop="storedDirName">
          <a-tree-select
...
import { saveImageManage, editImageManage, checkImageNameAvl, checkImageModel, newImage, getImageManageList, imageVersion } from "@/api/imageManage"
export default {
  name: "AddModelDialog",
  props: {
    dataSetTitle: {
  ...
  methods: {
    ...
    handleOk () {//点击确定触发
        ...
        if (!valid) {
            ...
            if (!that.rowData.id) {
                ...
                if (this.rowData.algorithmFetchType === 3) {
                  let name = this.modelInfo.find(item => item.id === this.rowData.algorithmStoredDir)
                  // this.rowData.parentVersionId = this.rowData.algorithmStoredDir
                  this.rowData.externalDepend = name?.modelName
                }
                if (!this.rowData.algorithmFetchType) this.rowData.algorithmFetchType = 2;
    ...
    algorithmTypeChange (e) {
      ...
      this.$set(this.rowData, 'algorithmStoredDir', '')
      // 更改算法代码的属性，触发该事件时需要讲算法URL切换对应的形式
      this.$refs.ruleForm.clearValidate(['algorithmUrl', 'algorithmName', 'fromImageId', 'algorithmStoredDir'])
    },
    ...
    async getChildren (val) {
      ...
      while (i < res.data.length) {
        if (res.data[i].imageStatus === 1) {
          ...
          this.$set(this.rowData, 'modelId', this.mirrorChildrenList[i].id)
          break
        } else {
          i++
        }
      ...
      if (this.rowData.algorithmFetchType === 3) {
        let res = await checkImageModel(val, { filter: 1 })
        if (res.code !== 0) return false
        this.modelInfo = res.data
        console.log(this.modelInfo, 'ppppppppppppppp');
      }
      this.$refs.ruleForm.clearValidate(['algorithmUrl', 'algorithmName', 'fromImageId', 'algorithmStoredDir'])
    }
  }
