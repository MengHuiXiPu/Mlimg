<template>
  <div class="mb-8"><i v-tooltip.right="name" class="fa fa-x" :class="icon" :style="{ color: iconColor, fontSize: '22px' }" @click="click"></i><br></div>
</template>
    ...
    execute() {
      ...
      canvas.toBlob(blob => {
        ...
        axios
          ...
          .catch(() => {
            this.axiosReqestError("Could not read data from URL");
          })
