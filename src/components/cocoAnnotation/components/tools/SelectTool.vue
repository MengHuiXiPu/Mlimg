<template>
  <div class="mb-8" :style="{ color: iconColor }" @click="click">
    <i class="fa fa-x icon-common" :class="icon" style="font-size: 18px"></i>
    <span class="text-desc">(Q){{ $t("dataCenter.select") }}</span>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      name: "Select",
      text: this.$t("dataCenter.selectTool"),
      cursor: "pointer",
      movePath: false,
      point: null,
      segment: null,
  ...
  methods: {
    ...
    generateTitle() {
      ...
      if (this.hover.category && this.hover.annotation) {
        ...
        let category = this.hover.category.name;
        string += "ID: " + id + " \n";
    ...
    checkBbox(paperObject) {
      ...
      let category = this.$parent.categoriesInstance;
      if (category) {
        let annotation = category.getAnnotation(annotationId);
        // if(annotation != null) {
        //   return annotation.annotation.iscrowd;
        // }
        return annotation?.annotation?.mode === 2;
      }
      return false;
    },
    onMouseDown(event) {
      ...
      if (hitResult.type === "segment") {
        ...
        // 禁用bbox拖拽调整path而修改
        paperObject = path.parent;
        if (!this.checkBbox(paperObject)) {
          // 下面这两行代码，在点击stroke附近时，会微调path，修改为仅在判定为非bbox时，才执行，否则会改变bbox形状
          let location = hitResult.location;
          this.segment = path.insert(location.index + 1, event.point);
        }
    ...
    onMouseMove(event) {
      ...
      if (hitResult) {
        ...
        if (hitResult.type === "segment") {
          ...
          const isBbox = this.checkBbox(hitResult?.item?.parent);
          // 当与strock碰撞时，仅在非bbox时才创建圆形标志路径(调用createPoint)
          point = isBbox ? null : hitResult.location.point;
        }
        if (point != null) {
      ...
      if (
        ...
        this.hover.category = this.$parent.categories[categoryId];

        if (this.hover.category != null) {
          // let annotation = category.getAnnotation(annotationId);
          this.hover.annotation =
            this.$parent.categoriesInstance &&
            this.$parent.categoriesInstance.getAnnotation(annotationId);
          event.item.selected = true;
          this.hoverText();
          if (this.hover.annotation) {
            const compoundPath = this.hover.annotation.compoundPath;
            compoundPath.onClick = () => {
              this.$emit("click", {
                annotation: annotationId,
                category: this.$parent.$refs.categories.index,
                keypoint: -1,
              });
            };
            compoundPath.on("mouseenter", function () {
              compoundPath.style = {
                strokeWidth: 3 * (this.scale || 1),
                strokeColor: "#7d0200",
                selectedColor: "#FFFFFF00",
              };
            });
            compoundPath.on("mouseleave", function () {
              compoundPath.style = {
                strokeColor: "#fff",
              };
            });
