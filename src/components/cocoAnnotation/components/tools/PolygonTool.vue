<template>
  <div>
    <div class="mb-8" :style="{ color: iconColor }" @click="click">
      <svg-icon class="icon" type="polygon" />
      <span class="text-desc">(R){{ $t("dataCenter.polygon") }}</span>
    </div>
    <modal-tool
      ref="toolModal"
      type="polygon"
...
export default {
  ...
  data() {
    return {
      ...
      color: {
        blackOrWhite: false,
        auto: true,
        radius: preferences.polygon.radius,
        circle: null,
      },
  ...
  methods: {
    ...
    setPreferences(pref, scale = 1) {
      ...
      const realScale = scale || this.scale;
      this.color.radius = (pref.radius || this.color.radius) * realScale;
    },
    /**
    ...
    onMouseDrag(event) {
      ...
      this.autoComplete(this.polygon.completeDistance, event.point);
    },
    onMouseDown(event) {
      if (this.isModalOpen) {
        return;
      ...
      if (this.pointCircle.length == 0) {
        ...
        if (!this.checkHit(event.point)) {
          this.pointCircle.push(startPointCircle);
          this.polygon.path.add(event.point);
        } else startPointCircle.remove();
      }
      if (this.autoComplete(this.polygon.completeDistance, event.point)) return;
    ...
    showStartPointCircle(point) {
      if (!this.startBlackPointCircle) {
        // 创建一个圆点并存储引用
        this.startBlackPointCircle = new paper.Path.Circle(
          point,
          this.color.radius * 1.3
        );
    ...
    setCircleRadius() {
      this.color.radius = preferences.polygon.radius * this.scale;
    },
    setPolygonCircleScale() {
      this.pointCircle = this.pointCircle.map((item) => {
  ...
  computed: {
    ...
    currentPaperScope() {
      return this.$store.state.annotator.paperScope;
    },
  },
  watch: {
    isActive(active) {
