<template>
  <span>
    ...
    <svg-icon v-if="item.isFolder" icon-class="folder" class="fiveEm"/>
    ...
    <div v-else-if="item.contentType.startsWith('image')">
      <el-image v-if="grid || grid === 'details'" :style="{'height':details?'110px':(gridWidth-35) + 'px'}"
                fit="contain" :src="imageSource(item)">
        <div slot="error" class="image-slot">
          <svg-icon icon-class="image" class=""/>
        </div>
      </el-image>
    ...
    <svg-icon v-else :icon-class="findIconClass" class="" />
    <!-- <svg>
      <use href="../../icons/svg/back.svg" />
    </svg> -->
  </span>
</template>
<script>
import {iconClass} from '../../utils/file-type'
// import '@/components/ShowFile/icons/index.js' // icon
// import SvgIcon from '../SvgIcon/index.vue'// svg component
import IconComponent from '../../icons/index.js'// svg component
export default {
  ...
  mounted() {
    console.log(this.grid,'pcccccccccccccccccc');
  },
  computed: {
    findIconClass() {
  ...
  methods: {
    imageSource(item) {
      if (item.cover) {
        ...
        return `data:image/jpeg;base64,${item.cover}`;
      } else {
