...
<template>
  <el-table ref="table" v-loading="loading" :data="data" v-bind="attrs" v-on="$listeners" :class="[type === 'authCode' ? 'authCode' : '']">
    <template v-for="column in mergedColumns">
      <el-table-column
        v-if="!column.hide"
        :key="column.prop"
        :type="column.columnType"
        :align="column.label == '操作' ? 'right' : 'left'"
        v-bind="column"
      >
        <template #header>
...
export default {
  ...
  computed: {
    attrs () {
      return {...this.$attrs, ...defaultTableAttrs}
    },
    mergedColumns () {
