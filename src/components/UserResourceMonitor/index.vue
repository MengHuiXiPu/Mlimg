...
export default {
  ...
  computed: {
    cpuPercentage () {
      ...
      return Math.round((this.resourceInfo.usedCpu / this.resourceInfo.hardCpu) * 100);
    },
    gpuPercentage () {
      if (!this.resourceInfo) {
        return 0;
      }
      return Math.round((this.resourceInfo.usedGpu / this.resourceInfo.hardGpu) * 100);
    },
    memPercentage () {
      if (!this.resourceInfo) {
        return 0;
      }
      return Math.round((this.resourceInfo.usedMemory / this.resourceInfo.hardMemory) * 100);
    },
    displayData () {
      const baseData = this.resourceInfo || defaultResourceInfo;
      const tasks = [];
